...
static void swe_sour(swe_solver *solver, physField *phys){
    ...
    const dg_real *f_Ds   = phys->cell->f_Ds;
    const dg_real *vgeo = phys->vgeo;
    ...
    for(k=0;k<K;k++){
        ...
        for (n=0;n<Np;++n) {
            ...
            const dg_real drdx  = vgeo[geoid++], drdy = vgeo[geoid++];
            const dg_real dsdx  = vgeo[geoid++], dsdy = vgeo[geoid++];

            const int ind = (k*Np+n)*Nfields;
            const dg_real h = f_Q[ind];
            const dg_real qx = f_Q[ind];
            const dg_real qy = f_Q[ind];
