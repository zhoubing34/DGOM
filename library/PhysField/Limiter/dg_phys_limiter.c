...
#include "PhysField/Limiter/dg_phys_indicator_all.h"
...
dg_phys_limiter* dg_phys_limiter_create(dg_phys_info *info){
    ...
    /* default indicator */
    limiter->indicator_func = dg_phys_indicator_all;
    /* limiter method */
...
static void dg_phys_set_indicator(dg_phys_limiter *limiter, Indicator_Type type){
    switch (type){
        case EDGE_INDICATOR: limiter->indicator_func = dg_phys_indicator_edge; break;
        case ALL_INDICATOR: limiter->indicator_func = dg_phys_indicator_all; break;
        default:
