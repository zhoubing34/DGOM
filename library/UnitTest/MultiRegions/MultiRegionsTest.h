...
do { \
    ...
    UnstructMesh *grid; \
    if (nprocs<2){ \
        grid = UniformTriMesh_create(2, 2, -1, 1, -1, 1, 1); \
    }else{ \
        grid = ParallelUniformTriMesh_create(2, 2, -1, 1, -1, 1, 1, procid, nprocs); \
    } \
    mesh = MultiReg2d_create(tri, grid); \
...
do { \
    ...
    UnstructMesh *grid; \
    if (nprocs<2){ \
        grid = UniformQuadMesh_create(2, 2, -1, 1, -1, 1); \
    }else{ \
        grid = ParallelUniformQuadMesh_create(2, 2, -1, 1, -1, 1, procid, nprocs); \
    } \
    mesh = MultiReg2d_create(quad, grid); \
    UnstructMesh_free(grid); \
}while(0) \
