...
nc_dim* nc_dim_create(char *namestr, int len){
    ...
    /* allocate name */
    dim->name  = (char *) calloc(strlen(namestr), sizeof(char));
...
void nc_var_print(nc_var *var){
    ...
    int i;
    for(i=0;i<var->ndim;i++){ nc_dim_print(var->dimarray[i]); }
    printf("\n");
}
...
nc_file* nc_file_create(char *namestr, int procid, int nprocs,
    ...
    int i;
    for(i=0;i<ndim;i++) {file->dimarray[i] = dimArray[i];}
    /* var */
    file->nvar     = nvar;
    file->vararray = (nc_var**) calloc(nvar, sizeof(nc_var*) );
    for(i=0;i<nvar;i++) {file->vararray[i] = varArray[i];}
