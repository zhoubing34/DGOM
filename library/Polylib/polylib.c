...
LIBRARY ROUTINES FOR ORTHOGONAL POLYNOMIAL CALCULUS AND INTERPOLATION
  ...
  laginterp   Returns lagrange interpolation value
 ...
 *
 * @details
 * The Lagrange interpolation function is derived by
 * \f[ L(x) = \sum_{k=0}^n l_k(x) y_k \f]
 * where \f$ l_k(x) \f$ is the Lagrange basis funcitions giving by
 * \f[ l_k(x) = \frac{ (x-x_0)(x-x_1)\cdots(x-x_{k-1})(x-x_{k+1})\cdots(x-x_n)}
 * { (x_k-x_0)(x_k-x_1)\cdots(x_k-x_{k-1})(x_k-x_{k+1})\cdots(x_k-x_n) } \f]
 *
 * The basis function fulfill Lagrange interpolation property
 * \f[ l_k(x_j) = \delta_{kj} = \left{
 * \begin{array}{ll} 1, & k=j \cr
 * 0, & k\ne j \end{array}
 * \right. \f]
 *
 * @param[in] nx0 number of input points
 * @param[in] x0 coordinate x of input points
 * @param[in] y0 coordinate y of input points
 * @param[in] nx number of interpolation points
 * @param[in] x coordinate x of interpolation points
 *
 * @return
 * return values:
...
void laginterp(int nx0, double *x0, double *y0, int nx, double *x, double *y){
  double l,z;
  int i, n, m;
