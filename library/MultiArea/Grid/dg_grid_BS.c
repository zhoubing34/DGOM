...
void dg_grid_init_BS(dg_grid *grid){
    ...
    for(k=0;k<K;k++){
        for(f=0;f<Nfaces;f++){
            ...
            if(EToP[k][f] != procid){ EToBS[k][f] = FACE_PARALL;
            }else{ EToBS[k][f] = FACE_INNER;
            }
        }
    }
    grid->EToBS = EToBS;
...
/**
 * @brief set boundary surface type into EToBS.
 *
 * @details
 * The SFToV (surface to vertex) matrix contains the vertex list
 * making up the surface (line) and the surface type.
 *
 * @param grid pointer to dg_grid structure;
 * @param Nsurface number of surface in SFToV;
 * @param SFToV surface type to vertex;
 */
