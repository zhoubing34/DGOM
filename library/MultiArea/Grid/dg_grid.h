...
typedef struct dg_grid{
    ...
    /** set boundary surface condition */
    void (*set_EToBS)(struct dg_grid *grid, int Nsurface, int **SFToV);
    /** project the grid vertex values to nodes */
    void (*proj_vert2node)(struct dg_grid *grid, int Nfield,
                           double *vertval, double *nodeval);
} dg_grid;


dg_grid* dg_grid_create(dg_cell *cell, int K, int Nvert,
                        double *vx, double *vy, double *vz, int **EToV);
