...
typedef struct dg_grid_creator{
    void (*set_vert)(dg_grid *grid, double *vx, double *vy, double *vz);
    void (*partition)(dg_grid *grid);
    void (*reset_EToV)(dg_grid *grid);
    void (*load_balance)(dg_grid *grid);
    void (*connect)(dg_grid *grid);
    void (*init_EToBS)(dg_grid *grid);
    void (*set_EToBS)(dg_grid *grid, int Nsurface, int **SFToV);
} dg_grid_creator;
...
/**
 * @brief create dg_grid structure with input grid geometry.
 * @details
 * the dg_grid contains
 *
 * @param[in] cell pointer to the dg_cell structure;
 * @param[in] K number of element;
 * @param[in] Nvert number of vertex;
 * @param[in] vx,vy,vz coordinate of vertex;
 * @param[in] EToV element to vertex list;
 *
 * @return
 * pointer to a new dg_grid structure.
 */
dg_grid* dg_grid_create(dg_cell *cell, int K, int Nvert, double *vx, double *vy, double *vz, int **EToV){
    ...
    switch ( dg_cell_celltype(cell) ){
        case TRIANGLE: creator = &grid_2d_creator; break;
        case QUADRIL: creator = &grid_2d_creator; break;
        default:
    ...
    int k,i,Nv = dg_cell_Nv(cell);
    grid->EToV = matrix_int_create(K, Nv); /* allocate and assign of EToV */
    for(k=0;k<K;k++){
        for(i=0;i<Nv;i++) { grid->EToV[k][i] = EToV[k][i]; }
    }

    grid->EToR = vector_int_create(K); /* allocation of EToR */
    creator->set_vert(grid, vx, vy, vz); /* copy vx,vy,vz into grid */
    creator->partition(grid);
    creator->reset_EToV(grid);
    creator->load_balance(grid);
    creator->connect(grid);
    creator->init_EToBS(grid);
    /* set methods for dg_grid */
    grid->set_EToBS = creator->set_EToBS;
 ...
 * @param[out] grid pointer to dg_grid structure;
 * @param[in] vx,vy,vz coordinate of vertex;
 */
static void dg_grid_set_vert2d(dg_grid *grid, double *vx, double *vy, double *vz){
...
//    dg_real initial_node_value[Np*Nfield];
    for(k=0;k<K;k++){
        ...
        cell->proj_vert2node(cell, Nfield, initial_vert_value, nodeval+k*Np*Nfield);
//        int sk = k*Np*Nfield;
