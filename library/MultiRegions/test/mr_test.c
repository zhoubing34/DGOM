...
dg_mesh *uniform_tri_mesh();
dg_mesh *uniform_quad_mesh();
dg_mesh *user_set_tri_mesh();
...
#define Nmesh 3
typedef dg_mesh* (* mesh_create_func)();
...
dg_mesh *uniform_tri_mesh(){
    ...
    dg_grid *grid = dg_grid_create_uniform_tri(shape, Mx, My, xmin, xmax, ymin, ymax, 1);
...
dg_mesh *uniform_quad_mesh(){
    ...
    dg_grid *grid = dg_grid_create_uniform_quad(shape, Mx, My, xmin, xmax, ymin, ymax);
    dg_region *region = mr_reg_create(grid);
    dg_mesh *mesh = mr_mesh_create(region);
    mr_mesh_add_bc2d(mesh, 0, NULL);
    return mesh;
...
void mesh_free(dg_mesh *mesh){
    dg_cell_free(mesh->cell);
    dg_grid_free(mesh->grid);
